...
class ParentController implements IController {
    ...
    initRoutes(): void {
        ...
        this.router.get('/parent/list/detailed', verifyToken, this.detailedParentList)
        this.router.post('/parent/ids', verifyToken, this.parentIds)
        this.router.post('/parent/upload', verifyToken, upload.single('file'), this.uploadParentsFromCSV);
        ...
        this.router.post('/parent/:id/students', verifyToken, this.changeParentStudents)
    }
    ...
    exportParentsToCSV = async (req: ExtendedRequest, res: Response) => {
        try {
            ...
            const csvRows = parents.map((parent: any) =>
                `${parent.email},${parent.phone_number},${parent.given_name},${parent.family_name}`
            );
            ...
            if (action === 'create') {
                for (const row of validResults) {
                    if (existingEmails.includes(row.email)) {
                        ...
                        const parent = await Parent.register(row.email);
                        await DB.execute(
    ...
    createParent = async (req: ExtendedRequest, res: Response) => {
        try {
            ...
            const parent = await Parent.register(email)
